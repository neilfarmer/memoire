...
analysis   = load_lambda("assistant", "analysis.py")
...
class TestMemoryProfile:
    def test_save_and_load_profile(self, tbls):
        memory.save_profile(USER, name="Neil", occupation="Engineer", summary="Builds things.")
        profile = memory.load_profile(USER)
        assert profile["name"]       == "Neil"
        assert profile["occupation"] == "Engineer"
        assert profile["summary"]    == "Builds things."

    def test_load_profile_empty(self, tbls):
        profile = memory.load_profile(USER)
        assert profile == {"name": "", "occupation": "", "summary": ""}

    def test_profile_fields_excluded_from_facts(self, tbls):
        memory.save_profile(USER, name="Neil", occupation="Engineer", summary="Builds things.")
        facts, _ = memory.load_memory(USER)
        assert "name" not in facts
        assert "occupation" not in facts
        assert "summary" not in facts

    def test_partial_save_only_updates_provided_fields(self, tbls):
        memory.save_profile(USER, name="Neil", occupation="Engineer", summary="Builds things.")
        memory.save_profile(USER, occupation="Designer")
        profile = memory.load_profile(USER)
        assert profile["name"]       == "Neil"
    ...
    def test_load_ai_analysis_empty(self, tbls):
        result = memory.load_ai_analysis(USER)
        assert result == {"analysis": "", "generated_at": ""}

    def test_ai_analysis_excluded_from_facts(self, tbls):
        memory.save_ai_analysis(USER, "Some analysis.")
        facts, _ = memory.load_memory(USER)
        assert not any("analysis" in k for k in facts)


# ── router: profile routes ────────────────────────────────────────────────────

class TestProfileRoutes:
    def test_get_profile_empty(self, tbls):
        resp = router.route("GET /assistant/profile", USER, {})
        assert resp["statusCode"] == 200
        body = json.loads(resp["body"])
        assert body == {"name": "", "occupation": "", "summary": ""}

    def test_put_and_get_profile(self, tbls):
        router.route("PUT /assistant/profile", USER, {"name": "Neil", "occupation": "Engineer", "summary": "Loves building."})
        resp = router.route("GET /assistant/profile", USER, {})
        body = json.loads(resp["body"])
        assert body["name"]       == "Neil"
        assert body["occupation"] == "Engineer"
        assert body["summary"]    == "Loves building."

    def test_put_profile_partial_update(self, tbls):
        router.route("PUT /assistant/profile", USER, {"name": "Neil", "occupation": "Engineer", "summary": "Loves building."})
        router.route("PUT /assistant/profile", USER, {"occupation": "Designer"})
        resp = router.route("GET /assistant/profile", USER, {})
        body = json.loads(resp["body"])
        assert body["name"]       == "Neil"
        assert body["occupation"] == "Designer"

    def test_put_profile_rejects_empty_body(self, tbls):
        resp = router.route("PUT /assistant/profile", USER, {})
        assert resp["statusCode"] == 400

    def test_get_memory_includes_profile_and_analysis(self, tbls):
        memory.save_profile(USER, name="Neil", occupation="Engineer", summary="Builds things.")
        memory.save_ai_analysis(USER, "Neil is focused.")
        resp = router.route("GET /assistant/memory", USER, {})
        body = json.loads(resp["body"])
    ...
    def test_generate_analysis_persists_and_returns(self, tbls):
        ...
        fake_resp = {
            "output": {"message": {"content": [{"text": "Neil is a focused engineer who values deep work."}]}},
        }
    ...
    def test_post_profile_analyze(self, tbls):
        ...
        with patch.object(analysis._bedrock, "converse", return_value=fake_resp):
            resp = router.route("POST /assistant/profile/analyze", USER, {})
        assert resp["statusCode"] == 200
        body = json.loads(resp["body"])
