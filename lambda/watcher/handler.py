...
def _infer_facts_from_activity(existing_facts: dict, activity_context: str) -> dict:
    ...
    prompt = (
        ...
        f"{activity_context}\n\n"
        "From this activity data, identify any NEW or UPDATED personal facts about this user "
        "(interests, hobbies, habits, goals, lifestyle, occupation, preferences, etc.).\n"
        "Only output facts that are clearly supported by the data and are stable personal attributes "
        "(not one-off tasks or temporary states).\n"
        "Output one fact per line as 'key: value' using short snake_case keys.\n"
        "If a fact already exists in 'Existing known facts' and the new data provides a better/updated value, "
        "include it with the updated value.\n"
        "If there are no new or updated facts, output exactly: NONE\n"
